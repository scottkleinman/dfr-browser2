...
Be careful! The metadata documents must be ordered in the same order as the documents imported into MALLET. If you used MALLET's `import-dir` flag, the documents were loaded in whatever order your system uses -- probably a combination of alphabetic and lexicographic order (1, 10, 2 instead of 1, 2 ... 10). We suggest importing your data into MALLET from a single file programmatically generating a file list so that you know what order your system uses to read a directory.
...
If you already have a JSON CSL-formatted bibliography that does not contain a `formatted_citation` field, you can use the `create_bibliography.py` script to add formatted citations based on your existing bibliography fields. Make sure the entries are in the same order as the metadata items.
...
- **topics.labels**: Override default topic numbering in the Overview topic bubbles grid with custom labels. The keys are currently zero-indexed, so "0" will be Topic 1 in the Topic menu. You can label some or all of the topics.
- **topics.hidden**: Hide specific topics from all views (can be toggled in Settings). The topic numbers are currently not zero-indexed, so 1, 2, 3 will remove the first three topics.

You may need to clear the cache for the changes to appear.
