...
async function loadDataSource () {
  ...
  try {
    ...
    const metadataResponse = await fetch(
      ensureAbsolutePath('sample_data/metadata.csv')
    )
    if (!metadataResponse.ok) {
      throw new Error(
        `Failed to load metadata.csv: HTTP ${metadataResponse.status}`
      )
    }
    ...
    const dataSourceColIndex = headers.findIndex(
      h => h.toLowerCase() === 'data_source'
    )

    if (dataSourceColIndex !== -1) {
      console.log(
        'Found data_source column in metadata, loading individual files...'
      )
      ...
      console.log(
        `Loaded ${
          dataSourceCache.filter(doc => doc.text).length
        } documents with text`
      )
    } else if (appConfig?.data_source) {
      console.log(
        'No data_source column in metadata, using config data_source file...'
      )
      ...
      console.log(
        `Loaded ${dataSourceCache.length} documents from single data source file`
      )
    } else {
...
function showDocumentText (documentId, documentText, docRef) {
  ...
  const modal = new bootstrap.Modal(
    document.getElementById('documentTextModal')
  )
  modal.show()

  document
    .getElementById('documentTextModal')
...
async function handleViewTextClick (referenceData) {
  ...
  try {
    ...
    let docId
    if (
      typeof referenceData.id === 'string' &&
      referenceData.id.includes('_')
    ) {
...
export async function loadDocumentView (
  ...
  if (referenceData && metadata && docTopic && topicKeys) {
    ...
    if (docIndex === -1 && referenceData.id) {
      docIndex = metadata.findIndex(
        doc => parseInt(doc.docNum) === parseInt(referenceData.id)
      )
      console.log(
        `Strategy 2 - Direct docNum match for ID ${referenceData.id}: index ${docIndex}`
      )
    }
    ...
    if (docIndex === -1 && referenceData.title) {
      docIndex = metadata.findIndex(
        doc =>
          doc.title &&
          doc.title.toLowerCase().includes(referenceData.title.toLowerCase())
      )
    ...
    if (docIndex !== -1 && docTopic[docIndex]) {
      ...
      totalTokens =
        docLengths && docLengths[docIndex] !== undefined
          ? docLengths[docIndex]
          : 0
      ...
      try {
        allEnhancedWords = await extractTopicWords(
          topicKeys.length,
          extractWords
        )
      } catch (error) {
      ...
      documentTopics = topicKeys.map((topic, topicIndex) => {
        ...
        const enhancedTopicWords =
          allEnhancedWords && allEnhancedWords[topicIndex]

        if (
          enhancedTopicWords &&
          Array.isArray(enhancedTopicWords) &&
          enhancedTopicWords.length > 0
        ) {
          const displayWordsArray = enhancedTopicWords.slice(0, displayWords)
          topWords =
            displayWordsArray.join(', ') +
            (enhancedTopicWords.length > displayWords ? '...' : '')
        } else {
          if (
            topic.words &&
            ...
            topWords =
              displayWordsArray.join(', ') +
              (topic.words.length > displayWords ? '...' : '')
          } else {
      ...
      console.log(
        `Found ${documentTopics.length} topics with non-zero proportions`
      )
    } else {
  ...
  if (referenceData) {
    ...
    if (referenceData.docIndex !== undefined) {
      try {
        const bibliographyPath =
          appConfig?.bibliography?.path || 'sample_data/bibliography.json'
        ...
        if (response.ok) {
          ...
          const bibEntry = bibliographyData.find(
            doc => doc._docIndex === referenceData.docIndex
          )

          if (bibEntry) {
            // Use the formatted citation from bibliography
            bibliographyCitation =
              bibEntry['formatted-citation'] || bibEntry._formattedCitation
          }
        }
      } catch (error) {
        // Bibliography not available, will fall back to metadata citation
        console.log(
          '[Document] Bibliography not available, using metadata citation'
        )
      }
    }
    ...
    // Priority 3: Construct citation from metadata fields
      ...
      if (referenceData.title)
        citationHtml += `<strong>${referenceData.title}</strong>`
    ...
    if (!appConfig?.embargo) {
      html += `<button type="button" class="btn btn-primary btn-sm" onclick="handleViewTextClick(${JSON.stringify(
        referenceData
      ).replace(/"/g, '&quot;')})">`
      html += `View Text`
      html += `</button>`
